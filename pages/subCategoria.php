...
<body class="hold-transition sidebar-mini layout-fixed">
        ...
        <div class="content-wrapper">
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <h3 class="card-title">Lista de SubCategorías</h3>
                    ...
                    <div class="modal-body">
                        <form class="form-inline" id="formulario" method="post">
                            <div class="input-group input-group-sm">
                                <div class="col-auto">
                                    <label for="Nombre" class="form-check-label">Nombre SubCategoría</label>
                                    <input id="Nombre" name="Nombre" class="form-control " type="text"
                                        placeholder="Nombre" aria-label="Nombre">
                                </div>
                                <div class="col-auto">
                                    <label for="Estado" class="form-check-label">Categoría</label>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        let tableBody = document.querySelector("#example1 tbody");

        let estadoCategoriaSelect = document.getElementById("categoriaLista");

        // Realizar una solicitud AJAX para obtener las categorías
        fetch('get/getCategoria.php')
            .then(response => response.json())
            .then(data => {
                // Iterar sobre los datos y agregar opciones al <select>
                data.forEach(categoria => {
                    let option = document.createElement("option");
                    option.value = categoria.cat_producto_num; // Valor del campo cat_producto_num
                    option.textContent = categoria.cat_prod_nombre; // Texto visible para el usuario
                    estadoCategoriaSelect.appendChild(option);
                });
            })
            .catch(error => {
        ...
        fetch('get/getSubCategoria.php')
            .then(response => response.json())
            .then(data => {
                // Iterar sobre los datos y agregar filas a la tabla
                data.forEach(categoria => {
                    var row = tableBody.insertRow();
                    row.insertCell().textContent = categoria.sub_cat_id;
                    row.insertCell().textContent = categoria.cat_prod_nombre;
                    row.insertCell().textContent = categoria.sub_cat_nombre;
                    row.insertCell().textContent = categoria.fecha_registro;
                    row.insertCell().textContent = categoria.estado_tipo;
                });
    ...
    function registrarCategoria(event) {
        ...
        let nombreCategoria = document.getElementById("Nombre").value;
        let categoria = document.getElementById("categoriaLista").value;
        let estadoCategoria = document.getElementById("estadoCategoria").value;

        if (nombreCategoria == "" || estadoCategoria == "" || categoria == "") {
            Toast.fire({
                icon: 'info',
            ...
            swal({
                    title: "¿Seguro que quieres registrar esta subcategoría?",
                    text: "Luego podrás editar la subcategoría!",
                    icon: "warning",
                    buttons: {
                        cancel: "Cancelar",
                        confirm: {
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        ...
                        fetch('create/createSubCategoria.php', {
                                method: 'POST',
                                body: JSON.stringify({
                            ...
                            .then(data => {
                                ...
                                if (data.success) {
                                    Toast.fire({
                                        icon: 'success',
                                        title: 'Se ha registrado la subcategoría con éxito!'
                                    });
                                    document.getElementById("formulario").reset();
                                } else {
                                    Toast.fire({
                                        icon: 'error',
                                        title: 'Error al registrar la subcategoría'
                                    });
                                }
                            })
                            .catch(error => {
